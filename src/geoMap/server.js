...
import fs from 'fs';
import { buildGeoData, getGeoRootDir } from './geoDataLoader.js';

const PORT = Number(process.env.GEO_MAP_PORT || 4177);
...
const server = http.createServer(async (req, res) => {
  ...
  if (url.pathname === '/') {
    sendHtml(res, mapHtml());
    return;
  }

  if (url.pathname === '/api/geo-data') {
    try {
      const data = await buildGeoData();
